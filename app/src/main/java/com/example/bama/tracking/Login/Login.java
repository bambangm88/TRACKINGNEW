package com.example.bama.tracking.Login;
...
public class Login extends AppCompatActivity {
    ...
    EditText txt_serial, txt_password;
    Intent intent;

    int success;
    ConnectivityManager conMgr;

    private String url = "http://bambangm.com/" + "login.php";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        serial = sharedpreferences.getString(TAG_serial, null);

        if (session) {
            Intent intent = new Intent(Login.this, MainActivity.class);
            intent.putExtra(TAG_ID, id);
            intent.putExtra(TAG_serial, serial);
            finish();
        ...
        btn_login.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (serial.trim().length() > 0 && password.trim().length() > 0) {
                    if (conMgr.getActiveNetworkInfo() != null
                            && conMgr.getActiveNetworkInfo().isAvailable()
                            && conMgr.getActiveNetworkInfo().isConnected()) {
                        checkLogin(serial, password);
                    } else {
                        Snackbar snackbar = Snackbar.make(findViewById(android.R.id.content),"Tidak Ada Koneksi Internet . .", Snackbar.LENGTH_SHORT);
                        snackbar.show();
    ...
    private void checkLogin(final String serial, final String password) {
        ...
        StringRequest strReq = new StringRequest(Request.Method.POST, url, new Response.Listener<String>() {
            ...
            public void onResponse(String response) {
                ...
                try {
                    ...
                    if (success == 1) {
                        String serial = jObj.getString(TAG_serial);
                       ...
                       Toast.makeText(getApplicationContext(), jObj.getString(TAG_MESSAGE), Toast.LENGTH_LONG).show();
                        ...
                        editor.putString(TAG_serial, serial);
                        editor.commit();
